...
def makeInline(a, format):
    '''
        Wählt nach dem Format die passende make(LaTeX/HTML)Inline
    '''
...
def isStringEndsWithSlash(str):
    return str[-1] == "/"


def isStringSlashOnly(str):
    return str == "/"
...
def handleStringPattern1(elem, doc):
    ...
    logging.debug("Pattern1 text: " + elem.text + " \t split: " + str(splt))
    if len(splt) == 4:
        if splt[3] == "":
...
def handleStringPattern6(elem, doc):
    ...
    if len(splt) == 4:
        ...
        logging.debug("Replacing "+elem.text+" to "+splt[1]+"(Halfspace)"+splt[2]+" at recomp6")
        return makeInline(splt[1:3], doc.format)
